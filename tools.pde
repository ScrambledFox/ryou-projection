...
void initMiddleware(){
  JSONObject json = loadJSONObject("setup.json");
  JSONArray devicesArray = json.getJSONArray("devices");
  
  ArrayList<Device> devices = new ArrayList<Device>();
  for(int i = 0; i < devicesArray.size(); i++){
    JSONObject jsonObj = devicesArray.getJSONObject(i);
    devices.add(jsonToDevice(jsonObj));
  }
  
  mw = new Middleware(devices);
}

Device jsonToDevice ( JSONObject obj ) {
  Device device = new Device();
  
  device.name = obj.getString("name");
  device.id = obj.getInt("id");
  
  JSONArray jsonEventArray = obj.getJSONArray("events");
  ArrayList<DeviceEvent> events = new ArrayList<DeviceEvent>();
  for(int i = 0; i < jsonEventArray.size(); i++){
    JSONObject jsonObj = jsonEventArray.getJSONObject(i);
    String eventName = jsonObj.getString("name");
    String eventType = jsonObj.getString("type");
    DeviceEvent event = new DeviceEvent(device, eventName, eventType);
    events.add(event);
  }
